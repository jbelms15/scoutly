...
type Product = { id: string; name: string; product_name: string; description: string; target_segments: string; key_differentiators: string; positioning_statement: string; active: boolean; sort_order: number }
...
function ProductsSection() {
  ...
  async function load() {
    const { data } = await createClient().from('kb_products').select('*').order('sort_order')
    if (data) { setItems(data); const m: Record<string, any> = {}; data.forEach(i => { m[i.id] = { ...i } }); setEditing(m) }
  }
  useEffect(() => { load() }, [])
  ...
  async function handleSave(id: string) {
    ...
    const ed = editing[id]
    await createClient().from('kb_products').update({ description: ed.description, positioning_statement: ed.positioning_statement, target_segments: ed.target_segments, key_differentiators: ed.key_differentiators }).eq('id', id)
    setSaving(s => ({ ...s, [id]: false })); setEditMode(m => ({ ...m, [id]: false })); load()
  }

  function upd(id: string, key: keyof Product, val: string) { setEditing(e => ({ ...e, [id]: { ...e[id], [key]: val } })) }
...
function ProofPointsSection() {
  ...
  async function handleAdd(e: React.FormEvent) { e.preventDefault(); await createClient().from('kb_proof_points').insert({ ...newItem, active: true, sort_order: items.length + 1 }); setNewItem({ headline: '', full_context: '', best_segments: '', case_study_company: '' }); setShowAdd(false); load() }
...
function CompetitorsSection() {
  ...
  async function handleAdd(e: React.FormEvent) { e.preventDefault(); await createClient().from('kb_competitors').insert({ ...newItem, name: newItem.competitor_name, differentiation: newItem.shikenso_differentiation, active: true }); setNewItem({ competitor_name: '', website: '', positioning_notes: '', shikenso_differentiation: '', when_likely_encountered: '' }); setShowAdd(false); load() }
...
// ─── Combined Shikenso Page ──────────────────────────────────────────────────

export default function ShikensoPage() {
  ...
  return (
    <div className="p-6 max-w-3xl space-y-4">
      <div>
        <h1 className="text-sm font-semibold text-fg">Shikenso</h1>
        <p className="text-xs text-fg-3 mt-0.5">Products, proof points, and competitor positioning — all pulled into Claude prompts dynamically.</p>
      </div>
      <Section title="Products" count={productCount}><ProductsSection /></Section>
