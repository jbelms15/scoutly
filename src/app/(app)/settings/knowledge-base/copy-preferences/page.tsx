...
type Pref = { id: string; preference_type: string; preference_value: string; active: boolean; version: number; updated_at: string }
...
export default function CopyPreferencesPage() {
  ...
  return (
    <div className="p-6 max-w-2xl space-y-4">
      <div>
        <h1 className="text-sm font-semibold text-fg">Copy Preferences</h1>
        <p className="text-xs text-fg-3 mt-0.5">Injected into every Claude copy generation call. Edit here — takes effect immediately on next generation.</p>
      </div>
          ...
          <div key={type} className="bg-surface border border-border rounded-xl p-5">
            <div className="flex items-start justify-between mb-3">
              ...
              <div className="flex items-center gap-2 shrink-0 ml-4">
                {pref && <span className="text-xs text-fg-3">v{pref.version}</span>}
